...
- `<语言>/guides/v1/`、`<语言>/guides/v2/`：版本概览与接入指南。
- `<语言>/updatelog/overview.mdx`：公共发布历史入口；`latest.mdx` 保留 V2.0.0 发布详情。
...
- 产品介绍与更新日志独立于版本；开发文档默认选择 V2，V1 保留查阅入口。
- 新版本的指南放入 `<语言>/guides/<版本>/`，参考内容放入 `<语言>/api-reference/<版本>/`，并在三种语言的开发文档导航中登记版本。
- 新版本内容完整并通过本地验证后，再调整默认版本。版本内链接指向明确版本。
- 现有错误码页面由 V1 与 V2 共同引用；出现版本差异时建立对应版本的说明，不要直接覆盖共享定义。
- 下线版本时先移除导航入口；删除旧页面时逐一处理旧链接，不将不同语义的接口统一重定向到新版本。
